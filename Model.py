...
def build_model(input_ph, batch_size, image_channels, sample_kind):
    ...
    kernel_1 = get_variable('Conv1-Kernel', [5, 5, image_channels, 8])
    ...
    conv_rlt_1 = tf.nn.leaky_relu(pre_act_1)
    # Pool & Norm 1
    #       'NHWC'
    ...
    # --------------------------------
    # Conv 2
    kernel_2 = get_variable('Conv2-Kernel', [3, 3, 8, 8])
    conv_2 = tf.nn.conv2d(norm_1, kernel_2, [1, 1, 1, 1], padding='SAME')
    bias_2 = get_variable('Conv2-Bias', [8])
    pre_act_2 = tf.nn.bias_add(conv_2, bias_2)
    conv_rlt_2 = tf.nn.leaky_relu(pre_act_2)
    # Norm & Pool 2
...
def train_operation(batch_size, image_width, image_height, image_channel, sample_kind):
    ...
    model = build_model(input_ph, batch_size, image_channel, sample_kind)
    ...
    return train_op, input_ph, label_ph, model
...
def evaluate_opration(path, image_width, image_height, image_channel, sample_kind):
    ...
    model = build_model(input_ph, 1, image_channel, sample_kind)
